      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="bg-white p-6 rounded-xl shadow-md border border-slate-200">
          <h3 className="text-base font-bold text-slate-800 mb-1 flex items-center gap-2">
            ...
            Havi teljesítmény
            <span className="ml-auto text-xs font-normal text-slate-400">utolsó 12 hónap, lezárt fuvarok</span>
          </h3>
          {monthlyData.length > 0 ? (
            <div className="space-y-1.5">
              <p className="text-[11px] text-slate-400 mb-3">Minden sor egy lezárt hónap összesítője. A sáv a havi maximumhoz viszonyított arányát mutatja.</p>
              {/* Header */}
              <div className="grid grid-cols-12 gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-wider px-1 pb-1 border-b border-slate-100">
                ...
                <div className="col-span-2 text-right">Fuvar (db)</div>
                <div className="col-span-3 text-right">Tonna (t)</div>
                <div className="col-span-3 text-right">Átlag t/fuvar</div>
                <div className="col-span-2">Arány</div>
              </div>
              {(() => {
                ...
                return monthlyData.map((m, i) => {
                  ...
                  const avg = m.darab > 0 ? (m.tonnas / m.darab) : 0;
                  return (
                    <div key={m.month} className={`grid grid-cols-12 gap-2 items-center px-1 py-1.5 rounded-lg transition-colors ${isLast ? "bg-purple-50 border border-purple-100" : "hover:bg-slate-50"}`}>
                      ...
                      <div className="col-span-3 text-right">
                        <span className="text-xs font-bold text-purple-600">{avg.toLocaleString("hu-HU", {maximumFractionDigits: 1})}</span>
                        <span className="text-[10px] text-slate-400 ml-0.5">t/fuvar</span>
                      </div>
