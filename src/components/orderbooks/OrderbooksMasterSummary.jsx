...
export default function OrderbooksMasterSummary({ orders, lines }) {
  ...
  // Only consider trucks belonging to open orderbooks
  const relevantTrucks = trucks.filter(t => openOrderIds.includes(t.orderbook_id));
  ...
  // Only open order lines
  const openLines = lines.filter(l => openOrderIds.includes(l.orderbook_id));
  ...
  const byCategory = useMemo(() => {
    ...
    // For each category, compute truck allocations proportionally
    const totalOrdered = openLines.reduce((s, l) => s + (l.planned_quantity_tons || 0), 0);
  ...
  }, [openLines, activeTrucks, closedTrucks]);
    ...
    <div className="space-y-5">
      ...
      <Card className="overflow-hidden">
        <div className="bg-gradient-to-r from-slate-700 to-slate-800 px-5 py-3 flex items-center justify-between">
          <div>
            <h3 className="font-bold text-white text-sm">Termékcsoportonkénti összesítő</h3>
            <p className="text-slate-400 text-xs mt-0.5">Összes nyitott rendelés · {openOrderIds.length} db rendelés</p>
          </div>
          ...
          <table className="w-full text-xs">
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        <div className="w-16 h-1.5 bg-slate-200 rounded-full overflow-hidden">
                          <div
                            className={`h-full rounded-full ${pct >= 100 ? "bg-emerald-500" : pct >= 70 ? "bg-blue-500" : pct >= 30 ? "bg-amber-400" : "bg-slate-300"}`}
                            style={{ width: `${Math.min(pct, 100)}%` }}
                          />
                        </div>
                        <span className={`font-bold text-[10px] w-7 text-right ${pct >= 100 ? "text-emerald-600" : pct >= 70 ? "text-blue-600" : "text-amber-600"}`}>
                          {pct}%
                        </span>
                      </div>
                    </td>
            ...
            <tfoot>
              <tr className="bg-slate-100 border-t-2 border-slate-300 font-bold text-slate-800">
                ...
                <td className="px-3 py-3 text-center text-slate-500 text-xs">{openOrderIds.length}</td>
...
function ProgressLegendItem({ color, label, tons, total }) {
  ...
  return (
    <div className="flex items-center gap-2 text-xs">
      ...
      <span className="font-bold text-slate-800">{tons.toLocaleString("hu-HU", { maximumFractionDigits: 1 })} t</span>
