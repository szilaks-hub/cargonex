...
export default function OrderbookDetail({ orderId, onClose, onUpdated }) {
  ...
  const [newLineForm, setNewLineForm] = useState(null);
  ...
  // Get filtered sites for current supplier
  const filteredSites = sites.filter(s => s.partner_id === form?.supplier_id);

  // Get selected supplier & site data
  ...
  const handleOpenOrder = async () => {
    if (!canOpenOrder) {
      toast.error('Supplier, site, and at least 1 line required');
      return;
    }
    ...
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <Card className="w-full max-w-6xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-gradient-to-r from-slate-50 to-white border-b p-4 flex items-center justify-between">
          <div>
            <h2 className="text-xl font-bold text-slate-800">{form.order_no || 'New Draft'}</h2>
            <p className="text-sm text-slate-500 mt-1">{selectedSupplier?.name || 'No supplier'}</p>
          </div>
        ...
        <div className="p-6 space-y-6">
          {/* Header Section - FULLY EDITABLE IN DRAFT */}
          <div className="space-y-4 p-4 bg-slate-50 rounded-lg border border-slate-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                {isDraft ? (
                  <Select value={form.supplier_id || ""} onValueChange={(v) => updateForm({ supplier_id: v, supplier_name: suppliers.find(s => s.id === v)?.name, supplier_site_id: "", supplier_site_name: "" })}>
                  ...
                  <div className="px-3 py-2 bg-white rounded border text-slate-800">{selectedSupplier?.name || '-'}</div>
                )}
              </div>
              ...
              <div>
                ...
                {isDraft ? (
                  <Select value={form.supplier_site_id || ""} onValueChange={(v) => {
                    ...
                    updateForm({ supplier_site_id: v, supplier_site_name: site?.location_name });
                  }}>
                  ...
                  <div className="px-3 py-2 bg-white rounded border text-slate-800">{selectedSite?.location_name || '-'}</div>
                )}
              </div>
              ...
              <div>
                ...
                {isDraft ? (
                  ...
                  <div className="px-3 py-2 bg-white rounded border text-slate-800">{form.currency}</div>
                )}
              </div>
              ...
              <div>
                ...
                {isDraft ? (
                  <Select value={form.incoterms_type || ""} onValueChange={(v) => updateForm({ incoterms_type: v })}>
                    <SelectTrigger className="bg-white"><SelectValue /></SelectTrigger>
                    <SelectContent>
                  ...
                  <div className="px-3 py-2 bg-white rounded border text-slate-800">{form.incoterms_type}</div>
                )}
              </div>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => updateForm({ incoterms_place: e.target.value })}
                  disabled={!isDraft}
                  className={!isDraft ? 'bg-slate-100 text-slate-600' : 'bg-white'}
                />
              </div>
            ...
            {selectedSite && (
              <div className="p-3 bg-blue-50 rounded border border-blue-200 text-sm">
                <div className="font-medium text-blue-900">Site Address</div>
                <div className="text-blue-800 mt-1">
                  {selectedSite.address && <div>{selectedSite.address}</div>}
                  <div>{selectedSite.postal_code} {selectedSite.city}, {selectedSite.region || selectedSite.country}</div>
                </div>
              </div>
            )}
            ...
            <div className="border-t pt-4 space-y-3">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  onChange={(e) => updateForm({ customs_required: e.target.checked })}
                  disabled={!isDraft}
                />
                  ...
                  <div>
                    ...
                    {isDraft ? (
                      <Select value={form.customs_agent_id || ""} onValueChange={(v) => {
                        ...
                        updateForm({ customs_agent_id: v, customs_agent_name: agent?.name });
                      }}>
                      ...
                      <div className="px-3 py-2 bg-white rounded border text-slate-800">{form.customs_agent_name || '-'}</div>
                    )}
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => updateForm({ customs_fee_eur_per_ton: e.target.value ? parseFloat(e.target.value) : null })}
                      disabled={!isDraft}
                      className={!isDraft ? 'bg-slate-100 text-slate-600' : 'bg-white'}
                    />
                  </div>
            ...
            {isDraft && (
              <div className="p-3 bg-blue-50 rounded border border-blue-200 space-y-2">
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
                  ...
                  <Button size="sm" className="bg-blue-600 hover:bg-blue-700 text-white md:col-span-1" onClick={handleAddLine}>Add</Button>
                </div>
              </div>
            )}
          </div>
      ...
      <Dialog open={!!confirmDialog} onOpenChange={() => setConfirmDialog(null)}>
        <DialogContent>
          ...
          <p className="text-sm text-slate-600">
            ...
            {confirmDialog?.action === 'close' && 'Order will move to Closed tab. You can reopen it later.'}
        ...
        <td colSpan="7" className="py-3 px-3">
          <div className="space-y-2">
            <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
              ...
              <Input type="number" placeholder="Qty (t)" step="0.01" value={form.planned_quantity_tons} onChange={(e) => setForm({ ...form, planned_quantity_tons: parseFloat(e.target.value) })} className="text-sm bg-white" />
              <Input type="number" placeholder="Price (EUR/t)" step="0.01" value={form.unit_price_eur_per_ton} onChange={(e) => setForm({ ...form, unit_price_eur_per_ton: parseFloat(e.target.value) })} className="text-sm bg-white" />
  ...
  return (
    <tr className="border-b hover:bg-slate-50">
      ...
      <td className="text-right py-3 px-3 space-x-1 flex justify-end">
        {isDraft && (
          <>
            <button onClick={() => onEditToggle(line.id)} className="text-blue-600 hover:text-blue-800 p-1"><Edit2Icon className="w-4 h-4" /></button>
